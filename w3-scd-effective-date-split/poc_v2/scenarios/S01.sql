...
SELECT 'S01 TC01 — stage' AS EVIDENCE, RULE_NO, ACTION_FLAG, RETIRE_MODE, DEL_IND,
       MERGE_KEY, BROKER_STATUS_CDE, COMMISSION_TIER_CDE, ROW_EFF_DTE, ROW_EXP_DTE
...
USING STG_Z2_BROKER_PARTY_DIM S
   ON T.BROKER_PARTY_DIM_SK = S.MERGE_KEY
WHEN MATCHED AND S.ACTION_FLAG = 'U' THEN UPDATE SET
     T.ROW_EXP_DTE = S.ROW_EXP_DTE, T.UUID = COALESCE(S.UUID, T.UUID),
     T.AUDIT_UPDATE_DATETIME = CURRENT_TIMESTAMP()
WHEN MATCHED AND S.ACTION_FLAG = 'D' THEN UPDATE SET
     T.IS_DEL = COALESCE(S.DEL_IND, T.IS_DEL),
     T.ROW_EXP_DTE = CASE WHEN T.ROW_EXP_DTE = DATE '9999-12-31'
...
SELECT 'S01 TC03 — stage' AS EVIDENCE, RULE_NO, ACTION_FLAG, RETIRE_MODE, DEL_IND,
       MERGE_KEY, BROKER_STATUS_CDE, COMMISSION_TIER_CDE, ROW_EFF_DTE, ROW_EXP_DTE
FROM   STG_Z2_BROKER_PARTY_DIM ORDER BY ROW_EFF_DTE, ACTION_FLAG;
